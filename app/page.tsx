...
import { Check, Zap, Crown, Bot, ArrowRight, LogOut, LayoutDashboard, Users, DollarSign, Lock, Sparkles, CircleAlert as AlertCircle } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Avatar, AvatarFallback, AvatarImage } from '@/components/ui/avatar';
import { checkAuthWithRetry, getSessionErrorMessage } from '@/lib/auth-utils';
...
export default function Home() {
  ...
  const checkAuthAndRedirect = async () => {
    ...
    if (fromDiscord) {
      ...
      try {
        const response = await checkAuthWithRetry(
          process.env.NEXT_PUBLIC_BACKEND_URL || '',
          { maxAttempts: 3, delayMs: 800, backoffMultiplier: 1.5 }
        );
        ...
        if (response.ok) {
          ...
          const errorMsg = getSessionErrorMessage(response.status);
        ...
        const errorMsg = getSessionErrorMessage(statusCode);
        setSessionError(errorMsg);
        setIsRedirecting(false);
      }
      return;
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-6">
          ...
          <h2 className="text-lg font-semibold mb-2">Verificando sesión...</h2>
          <p className="text-sm text-muted-foreground">Espera mientras establecemos tu sesión.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="max-w-md mx-auto px-6">
          ...
          <div className="text-center">
            <Button size="lg" onClick={handleDiscordLogin} className="gap-2 font-semibold">
