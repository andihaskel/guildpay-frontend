...
import { TestimonialModal } from '@/components/community/TestimonialModal';
...
export function PageTestimonialsSection({
  ...
  onChange: (items: PageTestimonial[]) => void;
}) {
  const [modalOpen, setModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<PageTestimonial | null>(null);
  ...
  function handleSubmit(item: Omit<PageTestimonial, 'id'> & { id?: string }) {
    if (item.id) {
      onChange(
        items.map(existing =>
          existing.id === item.id
      ...
      <div className="setup-incl-section">
        <div className="incl-head">
          <span className="incl-title">
            Testimonials <span className="count">{items.length}</span>
          </span>
          <span className="incl-hint">Shown in the carousel on the public page.</span>
        </div>
        ...
        {items.length > 0 ? (
          <div className="incl-list">
            {items.map((item, index) => (
              ...
              >
                <span className="incl-feat-icon">&ldquo;</span>
                <div className="incl-feat-body">
                  <span className="incl-feat-title">{item.quote}</span>
                  <span className="incl-feat-desc">
                    {item.author}
                    {item.since ? ` · Member since ${item.since}` : ''}
                  </span>
                </div>
          ...
          <div className="incl-empty">
            ...
            <span className="incl-empty-text">
              No testimonials yet. Add short quotes from members to build trust on your page.
            </span>
          </div>
        )}
